...
describe('API tests', () => {
    ...
    test('DELETE /api/vacations/listeners/:email', async() => {
        const listeners = await _fetch('get', '/api/vacations/listeners/')
        expect(listeners.length).not.toBe(0)
        // at this moment, all we can do is make sure the HTTP request is successful
        await _fetch('delete', `/api/vacations/listeners/${listeners[0].email}`)
    })
